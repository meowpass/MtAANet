...
from summation_version.Ablation_experiments.UNet_SegNet_CFF_DS import shared_encoder, seg_decoder, dose_decoder


class MT_PGAN(nn.Module):
    def __init__(self, gpu_ids=[], is_Train=True, continue_train=True):
        super(MT_PGAN, self).__init__()
        ...
        numGEnc = sum(p.numel() for p in self.netGEnc.parameters())
        numG4Seg = sum(p.numel() for p in self.netG4seg.parameters())
        numG4Dose = sum(p.numel() for p in self.netG4dose.parameters())
        numD = sum(p.numel() for p in self.netD.parameters())
        total = (numGEnc + numG4Seg + numG4Dose + numD) / (2 ** 20)
        ...
        if not self.isTrain or self.continue_train:
            print('----------load model-------------')
            save_dir = r'D:\code\MtAA_NET\multitask4segdose\UNet_SegNet_CFF_DS'
    ...
    def forward(self, origin, target, seg_GT):
        ...
        self.dose, self.DS4, self.DS3, self.DS2, self.DS1 = self.netG4dose(x5, x4, x3, x2, x1, d4, d3, d2, d1)
        self.target2 = interpolate(self.dose_GT, scale_factor=0.5, mode='bilinear')
        self.target3 = interpolate(self.dose_GT, scale_factor=0.25, mode='bilinear')
        self.target4 = interpolate(self.dose_GT, scale_factor=0.125, mode='bilinear')
    ...
    def optimizer_parameters(self, origin, target, seg_GT):
        ...
        self.Loss_DS = self.criterion4DS(self.target2, self.DS2) + self.criterion4DS(self.target3,
                                                                                     self.DS3) + self.criterion4DS(
            self.target4, self.DS4)
        self.loss_G = self.loss_segmentation + self.loss_prediction + self.Loss_DS
