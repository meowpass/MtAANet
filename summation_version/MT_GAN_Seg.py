...
from summation_version.Ablation_experiments.UNet_SegNet_CtFF import shared_encoder, seg_decoder, dose_decoder
# from summation_version.Ablation_experiments.UNet_SegNet import shared_encoder, seg_decoder, dose_decoder
...
class MT_PGAN(nn.Module):
    def __init__(self, gpu_ids=[], is_Train=True, continue_train=True):
        ...
        if not self.isTrain or self.continue_train:
            print('----------load model-------------')
            save_dir = r'D:\code\MtAA_NET\multitask4segdose\UNet_SegNet_CtFF'
    ...
    def forward(self, origin, target, seg_GT):
        ...
        self.seg = self.netG4seg(x5, x4, x3, x2, x1)
        self.dose = self.netG4dose(x5, x4, x3, x2, x1)
