...
class dose_decoder(nn.Module):
    def __init__(self, in_channel=1024):
        ...
        self.conv4 = nn.Conv2d(1024, 1, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv3 = nn.Conv2d(256, 1, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x5, x4, x3, x2, x1, d4, d3, d2, d1):
        x5 = self.conv_middle(x5)
        DS4 = self.sigmoid(self.conv4(x5))

        up_4_1 = self.att_block4(x5, x4)
        up_4 = torch.cat((up_4_1, d4), dim=1)
        up4 = self.channel_match1(up_4)
        DS3 = self.sigmoid(self.conv3(up4))

        up_3_1 = self.att_block3(up4, x3)
        up_3 = torch.cat((up_3_1, d3), dim=1)
        up3 = self.channel_match2(up_3)
        DS2 = self.sigmoid(self.conv2(up3))

        up_2_1 = self.att_block2(up3, x2)
        up_2 = torch.cat((up_2_1, d2), dim=1)
        up2 = self.channel_match3(up_2)
        DS1 = self.sigmoid(self.conv1(up2))

        up_1_1 = self.att_block1(up2, x1)
        up_1 = torch.cat((up_1_1, d1), dim=1)
        up1 = self.channel_match4(up_1)

        result = self.last_layer(up1)
        return self.sigmoid(result), DS4, DS3, DS2, DS1


class UNet_SegNet_CFF_DS(nn.Module):
    def __init__(self):
        super(UNet_SegNet_CFF_DS, self).__init__()
    ...
    def forward(self, x):
        ...
        seg_result, d4, d3, d2, d1 = self.seg_dec(x5, x4, x3, x2, x1)
        dose_result, DS4, DS3, DS2, DS1 = self.dose_dec(x5, x4, x3, x2, x1, d4, d3, d2, d1)
        return seg_result, dose_result, DS4, DS3, DS2, DS1


if __name__ == '__main__':
    x = torch.randn(2, 1, 512, 512)
    model = UNet_SegNet_CFF_DS()
    y, z, DS4, DS3, DS2, DS1 = model(x)
