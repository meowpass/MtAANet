...
from multitask4segdose.Dis import define_D
from multitask4segdose.GANLoss import GANLoss
from multitask4segdose.Imagepool import ImagePool
from multitask4segdose.proposed.MT_network import shared_encoder, seg_decoder, dose_decoder
...
class PGAN(nn.Module):
    def __init__(self, gpu_ids=[], is_Train=True, continue_train=True):
        ...
        if not self.isTrain or self.continue_train:
            print('----------load model-------------')
            save_dir = r'H:\code\MtAA_NET\multitask4segdose\checkpoints_Twin'
    ...
    def forward(self, origin, target, seg_GT):
        ...
        self.dose,self.up4 = self.netG4dose(x5, x4, x3, x2, x1, d4, d3, d2, d1)
