...
if __name__ == '__main__':
    ...
    for ii, batch_sample in enumerate(testData):
        ...
        x4_ = np.zeros(shape=(channel, 512, 512))
        up4_ = np.zeros(shape=(channel, 512, 512))
        for i in range(channel // batch):
            ...
            x4 = model.x4.squeeze(0)
            print("x4:",x4.shape)
            x41 = (x4[0].detach().cpu().numpy())
            print("x41:",x41.shape)

            up4 = model.up4.squeeze(0)
            up41 = (up4[0].detach().cpu().numpy())
